...
## Why Traffic Data? Climate Change!
...
bike_model_simple <-
  brm(count ~ temperature + windspeed + rain + month,
      family = negbinomial,
      data = bikes_neutor)
      
bikes_model_complex <- 
  brm(count ~ temperature * windspeed * rain * weekday * month,
      family = negbinomial,
      data = bikes_neutor)
```
...
```
  output$plotTemperature <- renderPlotly({
    p <- plot(
        marginal_effects(bike_model_simple,
                       effects = "temperature",
                       probs = c(0.05, 0.95),
                       plot = FALSE)
      )[[1]] + 
...
- TOOO
- Tobias Bradtke for the infrastructural support
